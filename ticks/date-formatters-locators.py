...
from matplotlib.dates import (
    AutoDateLocator, YearLocator, MonthLocator,
...
def plot_axis(ax, locator=None, xmax="2002-02-01", fmt=None, formatter=None):
    ...
    ax.text(
        0.0, 0.2, locator or formatter, transform=ax.transAxes,
        fontsize=14, fontname="Monospace", color="tab:blue",
    )


fig, ax = plt.subplots(
    len(locators), 1, figsize=(8, len(locators) * .8), layout="constrained",
)
...
fig, ax = plt.subplots(
    len(formatters), 1, figsize=(8, len(formatters) * .8), layout="constrained",
)
